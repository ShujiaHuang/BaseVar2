...
int main(int argc, char *argv[]) {
    ...
    // `pipeline` writes the generated shell script to stdout, so we MUST
    // keep stdout clean of the start/finish banners.  Detect it early and
    // dispatch directly without printing those wrappers.
    {
        std::string cmd0(argv[1]);
        if (cmd0 == "pipeline") {
            return basevar::pipeline::pipeline_runner(argc-1, argv+1, argv[0]);
        }
    }
    ...
    if (cmd == "caller") {
        ...
        run_stat = basevar::dump::dump_runner(argc-1, argv+1);
