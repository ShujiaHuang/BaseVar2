...
const std::string VCFSubsetSamples::usage() const {
    static const std::string BASETYPE_CALLER_USAGE =
        "Usage: basevar subsam [options] -i <input.vcf[.gz]> -o <output.vcf[.gz]> [-s <samplelist>] [<sample1> <sample2> ...]\n\n"

        "Options:\n"
        "  -i, --input FILE    Input VCF/BCF file (required).\n"
        "  -o, --output FILE   Output VCF/BCF file (required).\n"
        "  -s, --sample FILE   List of sample name to keep (one per line).\n"
        "  -O, --output-type   TYPE Output file type [v|z|b|u] (default: Guess format based on output file extension).\n"
        "                          v: VCF, z: compressed VCF (bgzip), b: BCF, u: uncompressed BCF.\n"
        "  --no-update-info    Do not update INFO fields (AC/AN/AF) based on extracted samples.\n"
        "  --keep-all-site     Do not remove the POS which only have reference allele in extracted samples.\n"
...
void VCFSubsetSamples::parse_args(int argc, char* argv[]) {
    ...
    _cmdline_string = "##basevar_subsam_command=";
    for (size_t i = 0; i < argc; ++i) {
        _cmdline_string += (i > 0) ? " " + std::string(argv[i]) : std::string(argv[i]);
    }

    int c;
    while ((c = getopt_long(argc, argv, "i:o:s:O:h", long_options, nullptr)) != -1) {
        switch (c) {
            case 'i': _input_vcf_path  = optarg;           break;
            case 'o': _output_vcf_path = optarg;           break;
            case 's': _samples_to_keep = 
                ngslib::get_firstcolumn_from_file(optarg); break;
